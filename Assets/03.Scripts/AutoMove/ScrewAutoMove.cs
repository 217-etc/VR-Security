...
public class ScrewAutoMove : MonoBehaviour
{
    ...
    private float rotationStep = 120f; // 자동 회전 각도 (시계 방향)
    ...
    void Update()
    {
        ...
        Debug.Log($"현재 회전 각도: {currentRotationY}, 이전 회전: {lastRotationY}, 차이: {rotationDiff}");

        // **시계 방향(반시계는 무시) & 최소 10도 이상 회전 시 감지**
        if (rotationDiff <= -requiredRotation)
        {
            Debug.Log("회전 감지: 시계 방향");
