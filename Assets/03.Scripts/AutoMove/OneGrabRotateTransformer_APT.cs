...
namespace Oculus.Interaction
{
    ...
    /// </summary>
    public class OneGrabRotateTransformer_APT : MonoBehaviour, ITransformer
    {
        public enum Axis
        {
            Right = 0,
        ...
        public void SetInitialAngleOnce(float currentAngle)
        {
            _maxAngle = currentAngle + 20f;

            Constraints.MinAngle.Constrain = true;
            Constraints.MaxAngle.Constrain = true;

            Constraints.MinAngle.Value = currentAngle;    // 초기 최소 각도 설정
            Constraints.MaxAngle.Value = _maxAngle;

            Debug.Log($"[초기 제한각도 설정] Min: {currentAngle:F2}, Max: {_maxAngle:F2}");
        }
        public void UpdateMinAngle(float currentAngle)
        {
            Constraints.MinAngle.Value = currentAngle;
            Debug.Log($"[각도 : MinAngle 갱신] 현재 Min: {currentAngle:F2}, Max: {_maxAngle:F2}");
        }
